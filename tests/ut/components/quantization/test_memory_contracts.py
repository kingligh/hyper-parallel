    ...
    def test_saved_tensors_are_released_after_backward(self) -> None:
        ...
        for grouped in (False, True):
            with self.subTest(grouped=grouped):
                ...
                node = y.grad_fn
                self.assertTrue(any(isinstance(t, torch.Tensor) for t in node.saved_tensors))
                self.assertFalse(any(isinstance(t, HiFloat8Tensor) for t in node.saved_tensors))
                y.sum().backward()
                with self.assertRaises(RuntimeError):
    ...
    def test_payload_lifetime_follows_retained_graph(self) -> None:
        ...
        for grouped in (False, True):
            with self.subTest(grouped=grouped):
                ...
                payloads = [weakref.ref(t) for t in saved[1:] if t is not None]
                del saved
