...
def validate_consistency_forward_inputs(
    ...
    *,
    ...
    try:
        ...
        expected_mask = (
            torch.arange(sequences.shape[1], device=sequences.device).unsqueeze(0)
            < lengths.unsqueeze(1)
        )
...
def _validated_logprob_mask(
    ...
    *,
    ...
    if actor_log_probs.dtype != rollout_log_probs.dtype:
        raise ValueError(
            f"{operation} log-probability dtype mismatch: "
            f"actor={actor_log_probs.dtype}, rollout={rollout_log_probs.dtype}"
        )
    if actor_log_probs.dtype != torch.float32:
        context = "consistency" if operation == "Pre-update" else "negative control"
        raise ValueError(
            f"{operation} {context} requires FP32 raw log-probabilities, "
            f"got {actor_log_probs.dtype}"
        )
    mask = experience.loss_action_mask.bool()
    if tuple(mask.shape) != tuple(actor_log_probs.shape):
        raise ValueError(
            f"{operation} action mask shape mismatch: "
...
def validate_pre_update_consistency(
    ...
    *,
    ...
    try:
        _populate_pre_update_record(experience, actor_log_probs, expected_policy_version, record, rank)
    except Exception as error:  # pylint: disable=W0718
    ...
    if group_size == 1:
        ...
        dist.all_gather_object(records, record, group)
...
def measure_post_update_old_policy_mismatch(
    ...
    *,
    ...
    mask = _validated_logprob_mask(experience, actor_log_probs, operation="Post-update")
    ...
    if group_size == 1:
        ...
        dist.all_gather_object(records, local_record, group)
...
def _populate_pre_update_record(experience, actor_log_probs, expected_policy_version, record, rank):
    ...
    mask = _validated_logprob_mask(experience, actor_log_probs, operation="Pre-update")
