...
def _install_batch_invariant_sum_compatibility() -> None:
    ...
    if _runtime.batch_invariant_sum_compatibility_installed:
        return
    try:
        from vllm_ascend import batch_invariant as batch_invariant_module  # pylint: disable=C0415
...
def _pack_trainer_inputs(sequences: Any, attention_mask: Any) -> tuple[Any, Any, Any, list[int]]:
    ...
    expected_mask = torch.arange(sequences.shape[1], device=sequences.device).unsqueeze(0) < lengths.unsqueeze(1)
    ...
    packed_position_ids = torch.cat(
        tuple(torch.arange(length, device=sequences.device) for length in lengths_cpu)
    ).unsqueeze(0)
    boundaries = [0]
...
def _restore_trainer_log_probs(sequences: Any, lengths_cpu: list[int], log_probs: Any) -> Any:
    ...
    for row, length in enumerate(lengths_cpu):
        ...
        rows.append(torch.cat((selected, selected.new_zeros((output_length - length + 1,)))).unsqueeze(0))
        start = end
...
def install_trainer_consistency_profile(config: Mapping[str, Any]) -> None:
    ...
    if (torch.accelerator.current_accelerator() or torch.device("cpu")).type != "npu":
        raise ValueError(f"Consistency profile {profile!r} requires the Torch NPU backend")
