...
class VLLMGenerationEngine:
    ...
    def generation_policy_version(self) -> int:
        """Verify and return the policy version served by vLLM."""
        return self._weight_sync.generation_version(self._ensure_client())
    ...
    def _server_endpoint(self) -> tuple[str, str, int]:
        ...
        if "LOCAL_RANK" not in topology_environment:
            topology_environment["LOCAL_RANK"] = str(dist.get_rank())
        if "LOCAL_WORLD_SIZE" not in topology_environment:
            topology_environment["LOCAL_WORLD_SIZE"] = str(dist.get_world_size())
    ...
    def _build_generation_result(
        ...
        sequences = torch.cat((request.input_ids, response_ids), dim=-1)
    ...
    def _generate(self, request: GenerationRequest) -> GenerationResult:
        ...
        client = self._ensure_client()
        worker_version = self._weight_sync.generation_version(client)
        result = None
        request_error = None
        try:
            result = self._generate_request(request, client, worker_version)
        except Exception as error:  # pylint: disable=W0718
            request_error = error
        self.synchronize_error(request_error, "generation request")
        if result is None:
            raise RuntimeError("vLLM generation request failed without a synchronized error")
        served_version = self._weight_sync.generation_version(client)
        if served_version != worker_version:
            raise RuntimeError(
                "vLLM policy version changed while serving a generation request: "
                f"before={worker_version}, after={served_version}"
            )
        return result
    ...
    def _validate_tp_request(self, request: GenerationRequest) -> None:
        ...
        dist.all_gather_object(contracts, contract, self._trainer_tp_group)
    ...
    def _generate_tp_owned(self, request: GenerationRequest) -> GenerationResult:
        ...
        if served_version != worker_version:
            raise RuntimeError(
                "vLLM policy version changed while serving a TP-owned request: "
                f"before={worker_version}, after={served_version}"
            )
        return self._broadcast_tp_result(request, result, worker_version)
    ...
    def synchronize_agent_payload(self, payload: Optional[Any]) -> Any:
        ...
        dist.all_gather_object(gathered, payload, self._trainer_tp_group)
...
def build_vllm_engine(
    ...
    return VLLMGenerationEngine(
        model,
        config,
        publisher=weight_transfer,
        rollout_model=rollout_model,
    )


async def _cancel_active_completions(active):
    """Cancel unfinished requests and drain every outstanding task on failure."""
