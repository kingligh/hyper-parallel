...
def _system_memory_metrics() -> dict[str, float]:
    ...
    handle = torch.get_device_module()
    ...
    return {
        "system/world_size": float(dist.get_world_size()),
...
def build_training_metrics(
    *,
    ...
    if policy is not None:
        ...
        configured_strategy = getattr(policy, "weight_sync_configured_strategy", None)
        last_strategy = getattr(policy, "weight_sync_last_strategy", None)
        for strategy in ("direct_reshard", "full_gather"):
            metrics[f"weight_sync/configured_{strategy}"] = float(
                configured_strategy == strategy
            )
            metrics[f"weight_sync/last_{strategy}"] = float(last_strategy == strategy)
...
def _probability_diagnostic_metrics(
    ...
    statistics = {
        name: _merge_statistics(records, name) for name in statistic_names
    }
    if any(value is None for value in statistics.values()):
        return None
    actor_log = statistics["actor_log"]
    rollout_log = statistics["rollout_log"]
    log_diff = statistics["absolute_log_diff"]
    probability_diff = statistics["absolute_probability_diff"]
    actor_probability = statistics["actor_probability"]
    rollout_probability = statistics["rollout_probability"]
...
def _add_target_diagnostics(
    ...
    distributions = (
        ("train/advantage", "advantages"),
        ("critic/return", "returns"),
        ("critic/value", "values"),
    )
    target_statistics = {
        name: _merge_statistics(records, name) for _, name in distributions
    }
    for prefix, name in distributions:
        _add_distribution_metrics(metrics, prefix, target_statistics[name])
    returns = target_statistics["returns"]
