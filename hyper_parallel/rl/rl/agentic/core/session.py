...
from rl.agentic.envs.environment import Environment
...
class AgentSession:
    ...
    def token_ids(self) -> Any:
        ...
        if self._cached_token_ids is None:
            self._cached_token_ids = torch.cat(self._token_parts)
    ...
    def _append_observation_turn(
        ...
        self._action_mask_parts.append(
            token_ids.new_zeros(token_ids.shape, dtype=torch.bool)
        )
        self._log_prob_parts.append(
            token_ids.new_zeros(token_ids.shape, dtype=torch.float32)
        )
    ...
    def record_worker_policy_version(self, version: Optional[int]) -> None:
        """Record one stable worker version across every generated turn."""
        if version is None:
            return
        version = int(version)
        if self._worker_policy_version is not None and version != self._worker_policy_version:
            raise RuntimeError(
                "AgentSession observed multiple rollout policy versions: "
                f"current={self._worker_policy_version}, received={version}"
            )
        self._worker_policy_version = version
    ...
    def build(self) -> Trajectory:
        ...
        action_mask = torch.cat(self._action_mask_parts)
        token_log_probs = torch.cat(self._log_prob_parts)
